  ...
  return (
    <li className={`timeline-entry quest-chain-entry tone-${presentation.tone}`} data-depth={depth}>
      ...
      <article className={`journal-card pixel-panel phase-${entry.status.toLowerCase()}`} id={`entry-${entry.id}`}>
        ...
        {entry.parent ? <p className={`chain-context ${entry.detachedFilterMatch ? "detached" : ""}`}>{entry.detachedFilterMatch ? <T k="journal.shownIndependently" /> : <T k="journal.partOf" />}<a href={`#entry-${entry.parent.id}`}>{entry.parent.title}</a></p> : null}
        ...
        {entry.children.length ? <p className="child-count">{entry.children.length} {entry.children.length === 1 ? <T k="journal.linkedEvent" /> : <T k="journal.linkedEvents" />}</p> : null}
        <footer className="entry-footer"><span>{entry.locationId ? <T k="journal.worldMapMilestone" /> : entry.origin === "SYSTEM" ? <T k="journal.recordedByWorld" /> : <T k="journal.writtenByYou" />}</span><div><JournalEntryDialog parent={{ id: entry.id, title: entry.title }} parentOptions={parentOptions} />{entry.origin === "MANUAL" ? <><JournalEntryDialog initial={initial} parentOptions={parentOptions} />{entry.status === "ONGOING" ? <CompleteEntryButton id={entry.id} /> : null}<DeleteEntryButton id={entry.id} /></> : null}</div></footer>
      </article>
